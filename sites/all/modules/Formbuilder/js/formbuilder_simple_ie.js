...
formbuilder.UI.showTab = function(type) {
	...
	$.post(homeurl+"formbuilder/views",{
		"action" : type,
...
formbuilder.forms.showProperties = function(formItem) {
	...
	if (typeof(form[formItem]['#label']) == 'undefined') {
		code += '<tr class="formbuilder_propertyRow"><td  title="Shows the type of element that is selected." class="formbuilder_propertyCell">Element <br />Type: </td><td>'+ form[formItem]['#type']+'</td></tr>';
	} 
	...
	if(typeof(form[formItem]['#button_type']) != 'undefined' )	{
		code += '<tr><td  class="formbuilder_propertyCell">Button type:</td><td><input type="text" name="button_type" size="5"  value="'+form[formItem]['#button_type']+'" onkeyup="formbuilder.forms.synchronize(\''+formItem+'\',\'#button_type\', 3)" /></td></tr>';
	}

	if(typeof(form[formItem]['#value']) != 'undefined' )	{
		code += '<tr><td  class="formbuilder_propertyCell">Value: </td><td><input type="text" name="value" size="5"  value="'+form[formItem]['#value']+'" onkeyup="formbuilder.forms.synchronize(\''+formItem+'\',\'#value\', 3)" /></td></tr>';
	}
...
formbuilder.forms.synchronize = function(formItem, key, source) {
	switch (key) {
		...
		case '#formbuilderName': {
			...
			delete form[formItem];
			//$('#'+formItem).id = newId; - This statement not replacing the actual element id.
			...
			//reframe the onmousedown attribute
			$('#'+newId).get(0).setAttribute("onmousedown", "formbuilder.forms.addItem('tempul', '"+newId+"', true, false)");	
			$('#close'+newId).get(0).setAttribute("onclick", "formbuilder.forms.removeItem('"+newId+"')");	
...
formbuilder.forms.editRequest = function(formid) {
	...
	$('#formbuilder_loadingAnimation').show();
	$.post(homeurl+"formbuilder/action",{
		"action" : 'edit',
		"formId" : formid,
		"rand" : Math.random()
	},
...
formbuilder.forms.manageForms = function(data) {
	...
	if(data) {
		...
		if ($('#formbuilder_displayPane').is(':visible')) {
			...
			$('#formbuilder_loadingAnimation').show();
			$.post(homeurl+"formbuilder/list",{"rand" : Math.random()},formbuilder.forms.manageForms);
		}
	}
}
...
formbuilder.forms.send =  function() {
  ...
  $("#formbuilder_loadingAnimation").show();
  $.post(homeurl+"formbuilder/receive",
  {
		"formName" : form["#id"],
