(function ($) {
	$.extend({
		survey_progressBar: new
		function () {
            var url = window.location.href;
            var urlpart = url.split("survey");
			...
			this.defaults = {
				...
				height: 12,
				callback: null,
				boxImage: urlpart[0] + 'sites/all/modules/survey/images/progressbar.gif',
				barImage: {
			...
			this.construct = function (arg1, arg2) {
				...
				return this.each(function (child) {
					...
					if (argvalue != null && this.bar != null && this.config != null) {
						...
						var $this = $(this);
						var config = $.extend({},
						$.survey_progressBar.defaults, argconfig);
						config.id = $this.attr('id') ? $this.attr('id') : Math.ceil(Math.random() * 100000);
						if (argvalue == null) argvalue = $this.html().replace("%", "");config.value = argvalue;
	...
	$.fn.extend({
		survey_progressBar: $.survey_progressBar.construct
	});
})(jQuery);