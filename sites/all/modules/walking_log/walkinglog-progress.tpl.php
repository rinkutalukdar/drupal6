	...
	<div class="walking-log-nav">
	    ...
	    <div class="progress-container"> 
				...
				<div class="print-progress-ctr">
					<a id="print_progress" target="_blank" href="<?php echo url("print/walking_progress")."?month=$month&type=$view"; ?>" ><img src="<?php echo url($path.'/images/print.png'); ?>" alt='print' class='my-progress-print'/></a>
				</div>
					...
					<script id="source">
						$(document).ready(function () {
							...
							var graph = $.plot($("#placeholder"), [d], { grid:{backgroundColor:"<?php echo $response['grid_background_color'];?>"},series:{lines: {show: true},color:"#FFFFFF"},xaxis: { mode: "time",timeformat: "%m/%d",labelWidth:"40" } });
							  ...
							  $("#placeholder #tooltip").mouseover(function() { 
								$("#placeholder .coll-out").css('display','block');
							  }).mouseout(function() {
								$("#placeholder .coll-out").css('display','none');
							  });
						...
						function showTooltip(x, y,left,top) {
							...
							var graphy = $('#placeholder').offset().top;
						      $('<div id="tooltip" style=""><img src="<?php echo $response['graph_status_image']; ?>" height="55px"/></div>').css( {
							   ...
							   $('<div class="coll-out"><img src="<?php echo $response['mood_image']; ?>" height="25px" /> <img src="<?php echo $response['weather_image']; ?>" height="25px"/></div>').css({
		      			          position: 'absolute',
		      			          top: y - 136,
			            		    left: x -54
       }).appendTo("#placeholder");
